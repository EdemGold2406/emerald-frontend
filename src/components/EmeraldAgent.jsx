...
const EmeraldAgent = () => {
  ...
  const [messages, setMessages] = useState([
    { role: 'assistant', text: "Hello! I am Emerald, your AI Copilot. How can I assist you today?" }
  ]);
  ...
  const handleSend = async (e) => {
    ...
    // 2. Simulate Backend AI Processing (We will connect real Node.js API here in Phase 2)
    setTimeout(() => {
      setIsTyping(false);
      ...
      if (lowerInput.includes('teacher') || lowerInput.includes('staff')) {
        navigate('/admin'); // Assuming they have access
        setMessages((prev) =>[...prev, { role: 'assistant', text: "Navigating you to the Teachers & Staff module now." }]);
      } 
      else if (lowerInput.includes('result') || lowerInput.includes('broadsheet')) {
        navigate('/admin');
        setMessages((prev) => [...prev, { role: 'assistant', text: "Opening the Exam Broadsheet Collation module for you." }]);
      }
      else {
        setMessages((prev) =>[...prev, { role: 'assistant', text: "I understand. Once connected to the backend, I will execute that command for you." }]);
      }
    }, 1500);
  };
    ...
    <>
      ...
      {isOpen && (
        <div className="fixed bottom-6 right-6 w-[350px] h-[500px] bg-white dark:bg-darkSurface border border-gray-200 dark:border-darkBorder rounded-2xl shadow-2xl flex flex-col z-50 overflow-hidden transition-colors animate-fade-in">
          ...
          {/* Header */}
          <div className="bg-emeraldGreen p-4 text-white flex justify-between items-center shadow-md z-10">
            <div className="flex items-center space-x-2">
              ...
              <div>
                ...
                <p className="text-[10px] opacity-80 flex items-center"><span className="w-2 h-2 bg-emeraldYellow rounded-full mr-1 animate-pulse"></span> Online</p>
              </div>
            </div>
          ...
          <div className="flex-1 p-4 overflow-y-auto bg-gray-50 dark:bg-[#0a0a0a] space-y-4">
            <div className="text-center text-xs text-gray-400 dark:text-gray-500 mb-4">
              Context: You are currently on {location.pathname === '/' ? 'the Home Page' : location.pathname}
            </div>

            {messages.map((msg, idx) => (
              <div key={idx} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                <div className={`max-w-[80%] p-3 rounded-2xl text-sm ${msg.role === 'user' ? 'bg-emeraldGreen text-white rounded-br-none shadow-md' : 'bg-white dark:bg-darkSurface border border-gray-200 dark:border-darkBorder text-gray-800 dark:text-gray-200 rounded-bl-none shadow-sm'}`}>
                  {msg.text}
                </div>
              </div>
            ))}
          ...
          <form onSubmit={handleSend} className="p-3 bg-white dark:bg-darkSurface border-t border-gray-200 dark:border-darkBorder">
            <div className="flex items-center space-x-2">
              <input
                ...
                placeholder="Ask Emerald to do something..."
