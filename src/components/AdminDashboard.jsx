        ...
        <nav className="space-y-4">
          {['students', 'teachers', 'classes', 'subjects'].map(m => (
            <button 
              ...
              className={`block w-full text-left font-bold uppercase text-[11px] ${activeTab === m ? 'text-emeraldGreen' : 'text-gray-400'}`}
            >
              {m}
            </button>
          ))}
                ...
                {teachers.map(t => (
                  <tr key={t.id} className="border-b dark:border-darkBorder">
                    ...
                    <td className="p-4 flex gap-2">
                      <button 
                        onClick={() => SchoolService.assignSubject(t.id, 1)} 
                        className="bg-emeraldGreen text-white px-2 py-1 rounded text-[10px] hover:opacity-80"
                      >
                        Assign Subjects
                      </button>
                      <button 
                        onClick={() => SchoolService.makeClassTeacher(t.id, 1)} 
                        className="bg-teal-600 text-white px-2 py-1 rounded text-[10px] hover:opacity-80"
                      >
                        Make Class Teacher
                      </button>
                      <button className="bg-blue-500 text-white px-2 py-1 rounded text-[10px]">Edit</button>
                    </td>
                  </tr>
                ))}
              </tbody>
